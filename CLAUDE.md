...
本项目使用本地参考文档（`docs/references/`）指导开发。

### 创意与设计阶段

**触发场景**：新服务 / 新功能 / 新接口 / 架构变更

1. 阅读 `docs/references/` 中相关参考文档
2. 编写设计文档放入 `docs/design-docs/`
...
1. 逐项对照 `docs/references/checklists/` 中对应清单
2. 执行命令：
   - `go test ./... -race -cover`
   - `go vet ./...`
   - `golangci-lint run`
...
2. 在未走完设计评审流程时修改架构层面代码（新 service / 分层变更 / 公共库）
