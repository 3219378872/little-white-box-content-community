...
- 行为分析聚合采用查询期视图而非 insert-triggered 物化视图：避免 at-least-once 投递
  在物化时重复计数（`xbh_analytics.sql` 注释）。代价是聚合结果受原始表 90 天 TTL 限制，
  不满足 REL-020 的“去标识聚合结果保留 365 天”；落地 365 天聚合需要定时聚合任务或
  等幂物化方案，属待设计决策项。
- 搜索 `Total` 只按本页过滤值回减，不重扫 ES。全库精确计数需要索引与权威完全同步，
  目前做不到，因此 DISC-001/CORE-015 仍标 partial。
- `CORE-013` 与 `CORE-062` 的冲突已于 2026-08-13 经人类采纳选项 B 收敛：
  `/api/v2/post` 写接口强制 `expectedRevision`（缺失/0 → 400 参数错误，冲突 → 409），
  `/api/v1` 写接口维持迁移期语义（缺失时跳过乐观锁）并登记废弃计划；
  新客户端一律使用 `/api/v2`。
