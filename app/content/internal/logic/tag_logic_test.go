...
func TestGetTagsLogic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(tm *MockTagModel) {
				tm.On("FindList", mock.Anything, 10).Return([]*model.Tag{}, fmt.Errorf("db error"))
			},
			wantErr: true,
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(pm *MockPostModel, ptm *MockPostTagModel) {
				...
				pm.On("FindByIds", mock.Anything, []int64{}).Return([]*model.Post{}, nil)
			},
			check: func(t *testing.T, resp *pb.GetPostsByTagResp) {
		...
		},
		{
			...
			setupMock: func(pm *MockPostModel, ptm *MockPostTagModel) {
				ptm.On("FindPostIdsByTagName", mock.Anything, "golang", 1, 10).Return(postIds, int64(2), nil)
				pm.On("FindByIds", mock.Anything, postIds).Return([]*model.Post{}, fmt.Errorf("db error"))
			},
			wantErr: true,
		},
		{
