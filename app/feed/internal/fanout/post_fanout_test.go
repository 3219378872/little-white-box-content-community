...
func TestHandlePostPublished_PaginatesFollowersUntilComplete(t *testing.T) {
	inbox := new(mockInboxModel)
	outbox := new(mockOutboxModel)
	userSvc := new(mockUserService)
	event := PostPublished{PostId: 1001, AuthorId: 9, CreatedAt: 1710000000000}
	userSvc.On("GetUser", mock.Anything, &userservice.GetUserReq{UserId: 9}).Return(&userservice.GetUserResp{User: &userservice.UserInfo{Id: 9, FollowerCount: 3}}, nil).Once()
	outbox.On("InsertIgnore", mock.Anything, &model.FeedOutbox{AuthorId: 9, PostId: 1001, CreatedAt: 1710000000000}).Return(nil).Once()
	userSvc.On("GetFollowers", mock.Anything, &userservice.GetFollowersReq{UserId: 9, Page: 1, PageSize: 2}).Return(&userservice.GetFollowersResp{Users: []*userservice.UserInfo{{Id: 1}, {Id: 2}}, Total: 3}, nil).Once()
	userSvc.On("GetFollowers", mock.Anything, &userservice.GetFollowersReq{UserId: 9, Page: 2, PageSize: 2}).Return(&userservice.GetFollowersResp{Users: []*userservice.UserInfo{{Id: 3}}, Total: 3}, nil).Once()
	inbox.On("BatchInsertIgnore", mock.Anything, mock.MatchedBy(func(rows []*model.FeedInbox) bool {
		return len(rows) == 3 && rows[0].UserId == 1 && rows[1].UserId == 2 && rows[2].UserId == 3
	})).Return(int64(3), nil).Once()

	pushed, err := HandlePostPublished(context.Background(), &svc.ServiceContext{InboxModel: inbox, OutboxModel: outbox, UserService: userSvc, BigVThreshold: 10000, FanoutBatchSize: 2}, event)
