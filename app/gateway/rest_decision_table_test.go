...
func TestRESTDecisionTable(t *testing.T) {
	...
	decisions = append(decisions,
		...
		restDecision{id: "USER-GET-AUTHENTICATED", method: http.MethodGet, path: "/api/v1/user/2", auth: true, wantStatus: http.StatusOK, wantHeaders: map[string]string{middleware.AuthStateHeader: middleware.AuthStateAuthenticated}},
		restDecision{id: "USER-POSTS-AUTHENTICATED", method: http.MethodGet, path: "/api/v1/users/2/posts?page=1&pageSize=20&sortBy=1", auth: true, wantStatus: http.StatusOK, wantHeaders: map[string]string{middleware.AuthStateHeader: middleware.AuthStateAuthenticated}},
		restDecision{id: "USER-FAVORITES-AUTHENTICATED", method: http.MethodGet, path: "/api/v1/users/2/favorites?page=1&pageSize=20", auth: true, wantStatus: http.StatusOK, wantHeaders: map[string]string{middleware.AuthStateHeader: middleware.AuthStateAuthenticated}},
