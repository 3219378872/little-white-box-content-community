...
func (m *agentCapabilityConsentModel) Get(ctx context.Context, userID int64) (*AgentCapabilityConsent, error) {
	...
	err := m.conn.QueryRowCtx(ctx, &row,
		"SELECT `granted`, `granted_at`, `revoked_at` FROM `agent_capability_consent` WHERE `user_id` = ? LIMIT 1",
		userID)
	if err != nil {
...
func (m *agentCapabilityConsentModel) Upsert(ctx context.Context, consent *AgentCapabilityConsent) error {
	...
	_, err := m.conn.ExecCtx(ctx,
		`INSERT INTO agent_capability_consent (user_id, granted, granted_at, revoked_at, updated_at)
		 VALUES (?, ?, ?, ?, NOW())
		 ON DUPLICATE KEY UPDATE granted = VALUES(granted), granted_at = VALUES(granted_at),
		   revoked_at = VALUES(revoked_at), updated_at = NOW()`,
		consent.UserID, boolToInt(consent.Granted), grantedAt, revokedAt)
	return err
}
