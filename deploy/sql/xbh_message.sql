...
CREATE TABLE IF NOT EXISTS `message` (
    ...
    `media_id` BIGINT DEFAULT NULL COMMENT '媒体消息引用的媒体ID（CORE-041）',
    `idempotency_key` VARBINARY(128) NOT NULL COMMENT '客户端幂等键，同一发送者范围内唯一',
    `status` TINYINT DEFAULT 0 COMMENT '状态 0:未读 1:已读',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_sender_idempotency_key` (`sender_id`, `idempotency_key`),
